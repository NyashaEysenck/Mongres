...
applicable, a documented live MongoDB test. Remaining `[ ]` items are required
to satisfy the original scope or to make its operational claims credible.

## A. Schema discovery

- [x] A command can connect to MongoDB and sample the configured active collection.
- [ ] A configured collection set is sampled, profiled independently, and exposed through one proxy process.
...
- [ ] Bulk or partial failures expose structured affected-count diagnostics beyond the current safe error message.
...
- [ ] A real LLM call selects a Rust-generated, lossless candidate for a genuine mixed-type write, and Rust executes only that validated candidate.
- [ ] Structural decisions remain reject-only unless each has a dedicated deterministic executor primitive and integration coverage.

## F. Installation and demo

- [ ] A clean-machine user can start the complete demo with documented Docker Compose commands; record a successful run.
...
- [ ] Startup includes proxy health/readiness checks and fails with an actionable message when MongoDB or the resolver is unavailable.
- [ ] Seed data includes a normal nested write case and a genuine mixed-type or structural ambiguity case.
- [ ] The documented demo shows schema discovery, a `psql` `SELECT`, a persisted nested write, and a mixed-type or structural ambiguity-resolved write.
