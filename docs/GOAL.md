...
Completion requires protocol evidence from `psql`, DBeaver, and one standard
PostgreSQL driver, including the supported bound-parameter flow. The proxy must
not rely on a custom MongoDB client or REST adapter for those clients.
...
5. Execute a write involving a genuine mixed-type or structural ambiguity, show the constrained LLM candidate decision, and verify the deterministic write result.

The first supported coercion must be a narrowly defined, Rust-owned,
lossless candidate selected by the LLM from an allowlist. Mixed shapes and
literal dotted-key execution remain fail-closed until each has a dedicated
deterministic execution primitive. The project must not claim full PostgreSQL
or full SQL compatibility. Unsupported syntax, unsafe coercions, and
unresolved document-shape ambiguity are expected to produce clear errors.
